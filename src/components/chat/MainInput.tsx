...
export default function MainInput({
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredFiles = files.filter((file) =>
      selectedModel.supportedFileTypes.includes(file.type)
    );
  ...
  useEffect(() => {
    ...
    const unsupportedFiles = selectedFiles.filter(
      (file) => !selectedModel.supportedFileTypes.includes(file.type)
    );
    ...
    if (unsupportedFiles.length > 0) {
      ...
      alert(
        "Some previously selected files are not supported by the new model and have been removed."
      );
    }
  }, [selectedModel]);
  ...
  return (
    ...
    >
      ...
      <input
        ...
        multiple
        accept={selectedModel.supportedFileTypes.join(",")}
      />

      <div className="flex justify-between">
        <Button
          disabled={!selectedModel.supportedFileTypes.length}
